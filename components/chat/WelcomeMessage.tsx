...
const WelcomeMessage: React.FC<WelcomeMessageProps> = ({
  ...
  return (
    <div data-testid='welcome-message' className='welcome-message py-12 text-center md:py-16'>
      <div className='welcome-message__icon mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-gray-800/80 shadow-lg ring-1 ring-cyan-500/30'>
      ...
      {sharedMode && (
        <div className='welcome-message__shared mb-6 inline-flex items-center gap-2 rounded-full border border-cyan-500/20 bg-gray-800/80 px-4 py-2 text-sm text-gray-300'>
          <span>💡</span>
          <span>分享的 AI 助理 - 您的對話不會永久儲存</span>
        </div>
      )}
      <div className='welcome-message__intro mx-auto max-w-xl rounded-2xl border border-gray-700/50 bg-gray-800/50 px-6 py-5 shadow-lg backdrop-blur-sm'>
        <p className='text-lg leading-8 text-gray-300'>
          {assistantDescription ? '讓我們開始聊天吧！' : '問我任何問題，我會幫助您！'}
        </p>
      </div>
      {starterPrompts.length > 0 && (
        <div className='welcome-message__prompts mx-auto mt-6 flex max-w-2xl flex-wrap justify-center gap-3'>
