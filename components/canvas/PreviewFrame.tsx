...
export function PreviewFrame({ preview }: PreviewFrameProps): React.JSX.Element {
  ...
  return (
    <div className='flex h-full min-h-0 flex-col gap-3 overflow-hidden'>
      {warnings.length > 0 && (
        <div className='rounded-xl border border-amber-400/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-100 break-words'>
          {warningsExpanded ? (
          ...
          {VIEWPORT_OPTIONS.map(option => {
            ...
            return (
              <button
                ...
                className={`inline-flex min-h-11 min-w-11 items-center justify-center rounded-md transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500/70 ${
                  isActive
