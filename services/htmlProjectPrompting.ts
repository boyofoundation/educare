...
export const SANDBOX_CAPABILITIES_PROMPT =
  'Preview sandbox capabilities: the rendered preview runs in a sandboxed iframe with a virtual file system (VFS) that fully supports multi-file ES modules (use relative specifiers like "./utils.js" or "/lib/helper.js" — circular imports work), image assets referenced by <img>/<picture>/<source> src or srcset, fetch() of project files (e.g. fetch("./data.json")), and CSS url()/@import references in both .css files and inline <style> blocks. Use project-relative paths for every local reference. Missing files and unresolved module specifiers surface as structured diagnostics (missing_reference / runtime errors) that you can read via getPreviewRuntimeErrors and self-repair from. writeFiles / replaceInFile / modifyLinesInFile also return `staticDiagnostics` whenever a written file has a syntax error (acorn for JS, css-tree/csstree-validator for CSS, parse5 for HTML, with inline <script>/<style> checked too); if you see that field populated, fix the listed syntax errors in the same turn before continuing — do not wait for a preview runtime probe.';

export const SANDBOX_BOUNDARIES_PROMPT =
  'Preview sandbox boundaries — these are NOT supported inside the sandbox: Web Workers and Service Workers (new Worker("./w.js") / navigator.serviceWorker cannot resolve against the sandbox), XMLHttpRequest for local files (use fetch() instead), dynamic import() whose specifier is built by string concatenation at runtime (only literal specifiers can be resolved), url() references inside inline style="..." attributes (move them into a <style> block or .css file), and external CDN resources require live network access. Bare module specifiers (e.g. import "react") will not resolve unless you bundle them. Prefer self-contained projects that rely only on browser-native features and the project files themselves.';

export const SANDBOX_ENVIRONMENT_PROMPT =
  'Preview sandbox environment (hard constraints you MUST design around): the preview is a SINGLE blob-URL document rendered inside <iframe sandbox="allow-scripts allow-forms allow-modals"> with an OPAQUE origin. Therefore: (1) Storage is ephemeral — localStorage, sessionStorage, and IndexedDB are usable, and you SHOULD use them when the feature calls for persistence (they persist normally once the project is exported or hosted on a real origin), but inside this preview the opaque origin gets a fresh storage partition on every preview rebuild/reload, so saved data does NOT survive. Whenever the project relies on storage, remind the user in your reply that data will not persist across reloads in the in-app preview. (2) Single-document navigation only — <a href> links to other project .html files are NOT rewritten and cannot navigate; build single-page experiences (JS-driven tabs/views/sections) and read other project files via fetch(). window.location is a blob: URL, so URL query params, path-based routing, and history.pushState/replaceState are unusable. (3) No popups, no top navigation, no downloads — window.open(), target="_blank", and <a download> are blocked by the sandbox (alert/confirm/prompt do work via allow-modals). (4) No server exists — form action/method submissions go nowhere; intercept submit events with preventDefault() and handle them in JS. (5) <base> tags are stripped at build time. (6) A JS file referenced by a classic <script src> tag is inlined and excluded from the module import map, so the same file cannot also be imported as an ES module — keep classic-script and module code in separate files. (7) console.error and console.warn calls are captured as runtime diagnostics and make getPreviewRuntimeErrors report has_errors, which blocks completion — remove debug logging that uses them before finishing.';
