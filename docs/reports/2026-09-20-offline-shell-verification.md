...
- Chromium／WebKit headless：12 項 E2E 通過，涵蓋新分頁冷開、未造訪設定頁、資產完整性、空快取首次離線、兩分頁 N→N+1、下載中斷及配額故障。
...
- 套用更新 API 已預設拒絕缺少儲存 guard 的操作；F1 草稿 flush、匯入／git barrier 及 F6 active-run guard 尚待接線，並補進行中工作案例。尚不能把套用更新按鈕視為完成驗收。
